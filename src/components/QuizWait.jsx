...
export default function QuizWait() {
  ...
  useEffect(() => {
    ...
    // Resetea la partida cada dia
    const today = new Date().toISOString().split('T')[0];
    const storedDate = localStorage.getItem('date');

    if (storedDate !== today) {
        localStorage.setItem('quizCompleted', 'false'); // Resetea el flag
        localStorage.setItem('date', today); // Actualiza la fecha
    }
...
✅ Aciertos: ${stats.successRate}%
  ...
  return (
    <div className="flex flex-col justify-center min-h-screen text-white px-2 pt-8 max-w-md w-full mx-auto">
      ...
      <h1 className="text-xl text-center text-white mb-6">La siguiente peli será mañana</h1>

      <h1 className="text-sm text-left text-slate-500 mb-2">TUS ESTADÍSTICAS</h1>
      ...
      {/* Alineación a la izquierda para el título y el contenido relacionado */}
      <div className="mb-10">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-sm text-left text-slate-500 mb-2">DISTRIBUCIÓN</h2>
        </div>
        <div className="space-y-2">
          {stats.distribution.map((count, index) => (
            <div key={index} className="flex items-center">
              ...
              <div
                className="bg-gradient-to-r from-purple-600 to-pink-500 h-6 rounded"
      ...
      <div className="mb-8">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-sm text-left text-slate-500 mb-2">SIGUIENTE PELI EN...</h2>
        </div>
      ...
      <button
        ...
        className="text-slate-500 border border-slate-500 px-6 py-3 rounded-full hover:bg-white hover:text-black transition duration-200"
      >
